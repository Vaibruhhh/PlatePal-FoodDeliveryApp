...
//     const listofRestro = await Restaurant.find({
//       $and: [
...
//       ]
//     })
//     .populate({path:'products'}) // Populate the 'restaurant' field with related documents
//     const neededList = listofRestro.reduce((ans,e1) => {
//       const newns =  e1.products.filter(e => e.name === item)
//       return [...ans,...newns]
//     },[])
...
app.get('/products/search',async (req, res) => {
  ...
  const neededList=listofRestro.filter((res)=>{
    const product=res.products.find((p)=>p.name.includes(item)|| (p.description&&  p.description.includes(item))
    )
     if(product){
      return true
     }
